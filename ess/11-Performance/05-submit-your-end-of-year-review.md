---
id: submit-your-end-of-year-review
summary:
  - "In ESS, go to **Skills and performance** and open your end of year review."
  - "Your goal setting and mid year comments are carried forward — add your end of year comments and final self-ratings."
  - "Submit the review. It goes to your manager, who completes it and submits it for calibration."
keywords: [end of year review, final self rating, complete status, carried forward comments, mid year comments, career aspirations, timeline, manager review pending, calibration, ESS]
video:
  youtube:
  bunny:
---

# Submit your end of year review

The end of year review closes your performance year. Everything from goal setting and the mid year review is carried forward, so you are making your final assessment against goals you have been tracking all year.

1. Open your review

   In ESS, go to **Skills and performance** and open your end of year review.

   ![The end of year review listed in Skills and performance](./images/submit-your-end-of-year-review-1.png)

   Your comments from goal setting and the mid year review are shown against each goal, along with your manager's, giving you the full year in one place.

2. Complete each goal

   Against each goal, add your end of year comments, set your final rating, and mark the goal's status — for example **Complete** where it has been achieved.

   ![A goal at end of year with the year's comments carried forward and the final rating set](./images/submit-your-end-of-year-review-2.png)

   Click **Next** to move through your goals.

3. Review career aspirations and training needs

   Your career aspirations and training needs are shown as they stand. Review them and make any final updates.

4. Check the timeline

   The timeline at the top of the review shows where you are in the year, with goal setting and the mid year review marked complete.

5. Submit the review

   Click **Submit**. Your end of year review is marked as submitted and the review moves to your manager, shown as manager review pending.

   ![The end of year review submitted, with manager review shown as pending](./images/submit-your-end-of-year-review-3.png)

   If you missed an earlier review this year, you can still complete your end of year review. The system does not block it where goal setting or the mid year review were not submitted.

6. Wait for your calibrated rating

   Your manager completes their feedback, gives an overall rating, and submits your review for calibration. Your final rating is confirmed through that process and published back to you — see [View your calibrated rating](./09-view-your-calibrated-rating.md).
