---
id: complete-an-onboarding-task
summary:
  - "From the ESS portal, navigate to **Tasks**."
  - "Locate your onboarding task, open it, and update the status in the dropdown to reflect completion."
  - "Click **Confirm** to record the update."
keywords: [onboarding, onboarding task, complete task, new employee, checklist, task status, task completion]
video:
  youtube:
  bunny:
---

# Complete an onboarding task

When you join GEMS, we automatically assign you an onboarding checklist. Complete each task in the list as you progress through the onboarding process.

1. Open your task list

   From the ESS portal, navigate to **Tasks**. Your onboarding tasks are listed here with their current status (Open, In Progress, or Completed).

   ![The Tasks screen showing a list of onboarding tasks with statuses including Open and Completed](./images/complete-an-onboarding-task-1.png)

2. Open a task

   Click on a task to view its details, including the task description and any dependencies that must be completed before you can progress.

3. Update the status

   Once the task is ready to be completed, update the status in the dropdown to reflect that it is done.

4. Confirm

   Click **Confirm** to record the update. The task is marked complete and removed from your active list.
