---
id: view-activities-calendar
summary:
  - "On the Activities overview, click the **Calendar view** card (or open **Calendar view** from the navigation)."
  - "Switch between **Month**, **Week** and **Day**, and page through dates."
  - "Activities show as coloured blocks; **click an event** to see its details."
keywords: [activities calendar, view calendar, check a date, excursion dates, activity dates, calendar view, month week day, see scheduled activities, clash check]
video:
  youtube:
  bunny:
---

# View the activities calendar

The activities calendar is a month, week or day view of scheduled activities —
handy for checking whether a date is free before you submit a request.

1. Open the calendar

   On the Activities overview, click the **Calendar view** card ("Check your
   preferred date before submitting request"), or open **Calendar view** from
   the navigation.

   ![The three action cards on the Activities overview, with the Calendar view card highlighted](./images/view-activities-calendar-1.png)

2. Choose how to view dates

   Switch between **Month**, **Week** and **Day**, and page forward or back
   through dates to find the period you need.

   ![The activities calendar in Month view — coloured activity blocks across the month, with the Month / Week / Day toggle at the top right](./images/view-activities-calendar-2.png)

3. Read an activity

   Activities are shown as coloured blocks, with the colour set by activity type.
   **Click an event** to open a pop-up with its name, type, dates, and the year
   group, house or class it's for.

   > **Note:** The calendar is read-only — you can't create or edit an activity from it; it's
   only there to help you pick an available date.
