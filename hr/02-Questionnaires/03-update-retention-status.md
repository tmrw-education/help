---
id: update-retention-status
summary:
  - "Open the employee's record in the **Employee Intention Survey Result** form."
  - "Click **Edit** and update the **Retention Status** dropdown."
  - "Optionally add a note in the **Comments** field, then **Save**."
keywords: [retention status, update retention, intention survey, HR discussion, retained, exit confirmed, in discussion]
video:
  youtube:
  bunny:
---

# Update an employee's retention status

As your retention conversation with an employee progresses, update their status in the Intentions Inquiry to keep the record current.

1. Open the intentions inquiry

   Navigate to **Human Resources ▸ Workers ▸ Inquiries and Reports ▸ Employee Intention Survey Result** and locate the employee.

2. Edit the record

   Click **Edit** to put the record into edit mode. Most fields are read-only. Only the **Retention Status** and **Comments** fields are editable.

   ![The Employee Intention Survey Result record with the Edit button highlighted in the Action Pane](./images/update-retention-status-1.png)

3. Update the retention status

   Select the appropriate status from the **Retention Status** dropdown:

   | Status | When to use |
   |---|---|
   | **In Discussion** | A retention conversation has been initiated |
   | **Retained** | The employee has agreed to remain |
   | **Exit Confirmed** | The employee is confirmed as leaving — triggers the recruitment process |

   ![The Retention Status dropdown open on an employee record, showing In Discussion, Retained, and Exit Confirmed options](./images/update-retention-status-2.png)

4. Add a comment

   Enter any relevant notes in the **Comments** field to document the outcome of conversations or actions taken.

5. Save

   Click **Save** to record the change.
