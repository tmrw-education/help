---
id: manage-contract-types
summary:
  - "Go to **Visa Master ▸ Contract Type** in D365."
  - "Review the existing types or click **New** to add one."
  - "Click **Save** when done."
keywords: [contract type, employment contract, full time, limited, part time, contract categories, visa contract]
video:
  youtube:
  bunny:
---

# Manage contract types

Contract types define the employment contract categories used when recording visa and employment information (for example: Full Time, Limited, Part Time). These values are configured once during initial setup and rarely change.

1. Navigate to Contract Type

   From D365, go to **Human resources ▸ Setup ▸ Visa Master ▸ Contract Type**. The configured contract types are displayed.

   ![The Contract Type list in D365 showing the configured contract type rows](./images/manage-contract-types-1.png)

2. Add a new type

   Click **New**, complete the required fields, then click **Save**.
