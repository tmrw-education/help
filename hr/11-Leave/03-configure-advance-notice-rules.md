...
summary:
  - "On the leave type, set the number of **consecutive days** that trigger the rule and the **days of notice** required."
...
2. Set the trigger and the notice period
   ...
   Together, these read: a request for 14 or more consecutive days of this type must be submitted at least 40 days before it starts.
...
4. Check the employee experience
   ...
   An employee whose request meets the day count but falls within the notice period sees the submission rejected with a message naming both values — *Leave requests of 14 days or more for this type must be submitted at least 40 days in advance* — and the request is not created.

   You can apply the rule to any leave type. Leave the values blank for types where no notice period applies.
