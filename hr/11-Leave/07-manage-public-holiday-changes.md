...
summary:
  - "In **Leave and absence ▸ Setup ▸ Calendars ▸ Calendars**, move the holiday to its new date and record a reason."
...
2. Open holidays and closures
   ...
   Open the **Holidays and closures** for that calendar. This is where you record announced holidays.
...
3. Move the holiday and record a reason
   ...
   Select the holiday, then click **Move holiday** to move it to its new date. Enter a reason for the change — for example, *Holiday moved due to announcement* — then click **OK**.
...
5. Confirm the system-generated requests are completed

   Each adjustment is raised as its own request with the **System generated leave request** flag set to **Yes**. Because the workflow condition auto-approves that flag, they complete without going to an approver. Give the process a few minutes, then refresh the time off requests to confirm the status has moved to completed.
...
6. Verify the balances
   ...
   Check the employee's balance (**Human resources ▸ Workspaces ▸ Employee self service ▸ Time-off requests**) as of a date after the change to confirm it reflects the adjustment. Balances are recalculated once the system-generated request completes, not when the holiday is moved.
