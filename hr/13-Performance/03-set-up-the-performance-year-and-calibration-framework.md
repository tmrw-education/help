...
summary:
  ...
  - "Open the calibration framework on that year and enter the expected percentage of employees in each rating band."
...
The performance year ties the whole cycle together — every review period and every generated review belongs to one. It also carries the calibration framework, which is the expected distribution of ratings HR compares actual results against during calibration.
...
1. Open the performance year form
   ...
   In D365, go to the **Performance year** setup under **Human Resources ▸ Performance**.
...
2. Create the year
   ...
   The start and end dates are yours to choose, so the performance year can follow the school year rather than the calendar year.

3. Open the calibration framework

   With the year selected, open the calibration framework.
...
4. Enter the expected distribution
   ...
   Set the framework before the end of year reviews are submitted. Calibration has nothing to compare against until these percentages exist.
