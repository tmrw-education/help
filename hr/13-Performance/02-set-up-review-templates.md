...
summary:
  - "In D365, go to **Human Resources ▸ Performance ▸ Setup▸ Review templates** and create one template per stage — goal setting, mid-year review and end-of-year review."
...
2. Create a template for each stage
   ...
   Create as many templates as needed. For example, create three templates — one for goal setting, one for the mid-year review, and one for the end-of-year review. Name each one for the stage it covers; you select the template by name when you generate reviews, so the naming needs to be unambiguous.

3. Add the goal templates

   Open the template and use the **+ Add goal template** button to bring in goal templates from your goal library. You are shown the list of goals already set up — select the ones this review should carry and click **OK**.

   Add every classification the stage needs. A goal-setting template, for example, typically includes both objectives and competencies.
...
5. Switch on the rating weight factor
   ...
   On the same tab, enable the rating weight factor. This allows the system to calculate an employee's **average score** and **total score** across the performance cycle — without it, the scores shown at the end-of-year review are not produced.

   ![The General tab showing the rating model and rating weight factor](./images/set-up-review-templates-3.png)

Review templates are only half the setup—the system also needs to know which year and period a review belongs to. See [Set up the performance year and calibration framework](./03-set-up-the-performance-year-and-calibration-framework.md).
